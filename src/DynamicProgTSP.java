...
public class DynamicProgTSP {
	...
	static final int[] MASKS = { 0, 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536,
	 ...
	 * 
	 * @param file
	 *            file specifying distances between cities
	 */
	...
	/**
	 * Instantiate TSP from provided adjacency matrix. The salesman will find
	 * the optimal route by which to visit each city, beginning and ending at
	 * city 0.
	 * 
	 * @param map
	 *            adjacency matrix of distances between cities
	 */
	 ...
	 * route is less than the known cost, known is updated. Otherwise, the
	 * current route is pruned.
	 * 
	 * @param open
	 *            cities not in current route
	 * @param visited
	 *            cities in current route
	 * @param route
	 *            current route as a stack
	 * @param ptr
	 *            stack pointer for current route
	 * @param d
	...
	void print_stuff(long t) {
		...
		if (VERBOSE) {
			...
			float mem = (0.125f * Float.SIZE * (map.length - 1) * MASKS[map.length - 1]) / (1 << 20);
	...
	/**
	 * Utility method to read input into an adjacency matrix.
	 * 
	 * @param file
	 *            input
