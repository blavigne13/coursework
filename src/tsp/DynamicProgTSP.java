...
public class DynamicProgTSP {
  ...
  float[][] map = null; // costs from i to j
  float[][] known = null; // best known from 0 to i, using visited
  int[] bestR = null; // best route
  float bestD = Float.MAX_VALUE; // best distance
  long cnt = 0; // visited nodes

  /**
   * Instantiate TSP from file. The salesman will find the optimal route by which to visit each
   * city, beginning and ending at city 0.
   * 
  ...
  /**
   * Instantiate TSP from provided adjacency matrix. The salesman will find the optimal route by
  ...
  /**
   * Invoking the run method causes Del Griffith to find the most efficient route for selling shower
   * curtain rings.
   */
  public void run() {
  ...
  /**
   * Execute recursive DFS with dynamic-programming-based pruning. As each successor is generated:
   * If the cost of visiting the cities on the current route is less than the known cost, known is
   * updated. Otherwise, the current route is pruned.
   * 
  ...
  void verify() {
    ...
    if (n < cees.length && (int) (bestD * 10) != cees[n]) {
      System.out.println("*************");
      System.out.println("Fail with n=" + n);
      System.out.println((int) (bestD * 10) + " != " + cees[n]);
      System.out.println("*************");
    }
  }
